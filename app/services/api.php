...
class API extends REST {
	...
	const DB_PASSWORD = "";
	const DB = "the_city";
	const GOOGLE_API_KEY = "AIzaSyBiB9CCaXXXXXXXXXXXXXXXXXXXXXXXXXXX";
	...
	private function getPlaces(){
        ...
        $query = $query . "ORDER BY p.last_update DESC;";
		$this->get_list($query);
	}
	...
	private function sendNotif() {
		...
		foreach($gcmRegIds as $val){
			$pushStatus[] = $this->sendPushNotification($val, $notif_title, $notif_content, $notif_place);
		}
	...
	private function sendPushNotification($registatoin_ids, $title, $content, $place){
		// Set POST variables
		$url = 'https://android.googleapis.com/gcm/send';
		$fields = array(
			'registration_ids' => $registatoin_ids,
			'data' => array( 'title' => $title, 'content' => $content, 'place' => $place, )
		);
